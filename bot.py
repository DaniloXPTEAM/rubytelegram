...
def main():
    ...
    # ✅ INICIALIZAÇÃO CORRETA do python-telegram-bot:
    # sem initialize()/start(), o process_update NÃO funciona (falha silenciosa).
    import asyncio
    try:
        asyncio.get_event_loop().run_until_complete(app_telegram.initialize())
        asyncio.get_event_loop().run_until_complete(app_telegram.start())
        logger.info("Application do Telegram inicializada e iniciada.")
    except Exception as e:
        logger.error(f"Falha ao inicializar app do Telegram: {e}")
    ...
    def telegram_webhook():
        ...
        try:
            ...
            # processa de forma síncrona (sem coroutine pendurada)
