...
def main():
    ...
    # ============ ARQUITETURA CORRETA: Flask em THREAD + loop do Telegram dedicado ============
    # O python-telegram-bot v20 é async: precisa de um event loop que fique VIVO
    # rodando em paralelo ao Flask. Sem isso, process_update não funciona.
    import asyncio
    ...
    # Rota do webhook do Telegram no Flask
    ...
    def telegram_webhook():
        ...
        try:
            ...
            # Agenda o processamento no loop do bot (thread-safe)
            asyncio.run_coroutine_threadsafe(
                app_telegram.process_update(update), loop
            )
    ...
    # Mantém o loop do bot vivo em background
    def rodar_loop():
        loop.run_forever()
    Thread(target=rodar_loop, daemon=True).start()

    # Inicia o Flask no thread principal (bloqueia)
